...
interface ParaglidingEarthApi {
    suspend fun getLaunchSitesInBounds(
        ...
        east: Double,
        limit: Int = 150,
        style: String = "detailled",
    ): ParaglidingEarthFeatureCollection
}

class KtorParaglidingEarthApi(
    private val httpClient: HttpClient,
    baseUrl: String = PARAGLIDING_EARTH_BASE_URL,
) : ParaglidingEarthApi {
    private val boundingBoxEndpoint =
        "${baseUrl.trimEnd('/')}/geojson/getBoundingBoxSites.php"
    ...
    override suspend fun getLaunchSitesInBounds(
        ...
        return executeJsonRequest {
            httpClient.get(boundingBoxEndpoint) {
                parameter("north", north)
                parameter("south", south)
                parameter("west", west)
                parameter("east", east)
...
const val PARAGLIDING_EARTH_BASE_URL = "https://www.paragliding.earth/api"
